...
```
cd 01-message-pump
go run ./cmd/receiver  # terminal 1: the pump
go run ./cmd/sender    # terminal 2: sends one order
```
...
| file | |
|---|---|
| `simplemessaging/messagepump.go` | the pump: **Get → Translate → Dispatch → Handle** |
| `model/placeorderhandler.go` | your application code |
| `model/placeorderhandler.go` | **the same file again** — this time read only its `import` block |

1. **Find the four stages of the pump in `messagepump.go`.** Get is labelled. Where are the
   other three?
2. **`PlaceOrderHandler.Handle` takes a `amqp091.Delivery`.** What is a `amqp091.Delivery`, which
   package does it come from, and what is it doing in a method about placing orders?
3. **`model/placeorderhandler.go` imports `github.com/rabbitmq/amqp091-go`.** `model` is the domain.
...
## PROBE B — a handler that fails — 5 minutes ##

```
go run ./cmd/sender poison
```

That order is **perfectly well formed**. It maps. `NOPE-404` is simply not in the catalogue,
so `Catalogue.PriceOf` fails — the way a real lookup fails when a real service is having a
real problem.
...
```
go run ./cmd/sender  # a perfectly good WIDGET-1
```
...
`Handle(amqp091.Delivery)` becomes `Handle(PlaceOrder)`. When you are done:

- **Nothing under `model/` imports `github.com/rabbitmq/amqp091-go`.** Delete that import from
  `model/placeorderhandler.go` and make the build pass. That is your check, it is a compile
  error rather than a matter of opinion, and the line it points at is the handler's
  signature — which is the line this exercise is about.
- **Go has no way to say "this package may not see that dependency"**, the way a `.csproj` or
  a `pom.xml` can. What it has is the import list and a compiler that will not let you name a
  package you did not import. That is enough, and it is checked the same way: build it.

**3. Move the acknowledgement.** The pump acks a message it has not finished with. Decide where
the ack belongs, and what should happen instead when the stage before it fails.

**And when all three are done, delete the banner at the top of `simplemessaging/messagepump.go`.** It says the
...
and a warning that has stopped being true is worse than no warning.
  ...
  - For a handler that failed — is that the same answer?
...
- Send `go run ./cmd/sender burst 50` and watch the queue drain. Then start a
