...
`model/catalogue.go` is a `map[string]float64`. It has been standing in for reference
...
4. **`Catalogue` reads the local copy** instead of its dictionary. `PriceOf` no longer returns
   `ErrCatalogueUnavailable`, because there is nothing left to be unavailable.

**SQLite is the cheapest durable store there is**, and that is the whole reason it is the one
specified: the database is a file you can delete, with no container and no port. The driver
is `github.com/mattn/go-sqlite3`, which needs cgo -- and exercise 3 already needed cgo for Kafka, so it
costs you nothing you have not already paid. `modernc.org/sqlite` is the pure-Go alternative
...
implements it, and is the only place that names SQLite. `cmd/receiver/` puts the two together,
...
**And it is a check rather than a matter of opinion.** It is the import block again, exactly
as it was in exercise 1: nothing under `model/` imports `github.com/mattn/go-sqlite3`,
`localcopy/` is the only package that does, and the compiler will not let you name a
package you did not import. Add the import to the domain to make an error go away and you have
made the mistake.

### The `PlaceOrderHandler` must not change either ###

If it does, the lookup has leaked into your domain the way `amqp091.Delivery` did in exercise 1 —
...
```
go run ./cmd/priceconsumer     # terminal 1, leave it running
go run ./cmd/receiver          # terminal 2, leave it running
go run ./cmd/priceseeder seed  # terminal 3: publish a starting price for every SKU
go run ./cmd/sender            # terminal 3: place an order
```
...
```
go run ./cmd/priceseeder set WIDGET-1 11.99  # terminal 3, watch terminal 1 print the gap
go run ./cmd/sender                          # then place an order and see which price it got
```
...
go run ./cmd/priceseeder set WIDGET-1 49.99
go run ./cmd/sender
```
...
```
go run ./cmd/sender         # WIDGET-1: fine, but we have no copy yet
# ...now start the price consumer and seed it, then...
go run ./cmd/sender poison  # NOPE-404: not a thing, and never will be
```
...
```
PRICE_WRITE_WINDOW=15 go run ./cmd/priceconsumer  # terminal 1
go run ./cmd/priceseeder set WIDGET-1 77.77       # terminal 3
kill -9 <the price consumer's PID>                # while the window is open
go run ./cmd/priceconsumer                        # start it again and watch
```
...
**Now reverse the two lines in `cmd/priceconsumer/main.go`** — commit the offset first, then apply —
and run it again.
