...
const (
	...
	// PriceConsumerGroup is the price consumer's own group. It reads a different topic for
	// a different reason, so it gets a group of its own: two consumers in one group would
	// be told to share the partitions of everything the group subscribes to, which is not
	// what either of them wants -- and the offsets of streams.OrderPlaced and
	// streams.PriceChanged have nothing to do with each other.
	//
	// A consumer group is a unit of work-sharing, not a name for your application.
	PriceConsumerGroup = "practical-messaging-prices"
