...
The pump had one `if err != nil`, covering the mapper and the handler together, and one
response to it: `Requeue`, which is `basic.nack` with `requeue: true`.
...
3. **It does not distinguish the two failures.** A body that cannot be mapped and a handler that
   failed arrive at the same `if err != nil` and leave by the same door — and one of them can
   never succeed, so retrying it is the definition of a poison pill.

▎ *A message that is only ever nacked blocks the queue forever. Requeue and reject both have to end somewhere.*

## The fix ##

**Two branches, because there are two failures.**

Go gives you no type to catch, so the question "which failure is this?" is asked with
`errors.Is(err, ErrUnmappableMessage)` instead. **The mechanism is gone and the distinction is
not** — which is the part worth carrying away, because the distinction is the lesson and the
`catch` was only ever the syntax.

### A failure to understand: `ErrUnmappableMessage` ###
...
**You wrote a policy — about twenty lines of error handling — and RabbitMQ did all the work.** No
