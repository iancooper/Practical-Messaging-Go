...
- `PlaceOrderHandler.Handle` takes a `PlaceOrder`. `model/placeorderhandler.go` has no broker reference
...
```
cd 02-failing-well
go run ./cmd/receiver  # terminal 1
go run ./cmd/sender    # terminal 2
```
...
|---|---|
| `simplemessaging/channel.go` | the topology, drawn in the comment. Read the comment |
| `simplemessaging/consumer.go` | the five things the pump can do with a message |
| `simplemessaging/messagepump.go` | the error handling at the bottom of the loop. All of it |
...
3. **The pump has one `if err != nil`, and every `error` gets the same answer.** Name two
   failures that both land in it and should not be treated the same way.

▎ *Dead Letter: we could not deliver it. Invalid Message: we delivered it and could not read it.* The pump has one error path. It therefore has one answer to two questions.
...
```
go run ./cmd/receiver | tee /tmp/probe-a.log  # terminal 1
go run ./cmd/sender unmappable                # terminal 2
```
...
```
go run ./cmd/sender poison  # NOPE-404: never in the catalogue, ever
```
...
8. NOPE-404 and FLAKY-1 fail from the same line of the same method.
...
**One file: `simplemessaging/messagepump.go`.** The topology is already declared and correct;
...
**Split the one error path into two, because you have two kinds of failure** — and
`errors.Is` is how you tell them apart:

**A failure to *understand*** — the mapper returned `ErrUnmappableMessage`. The bytes are
...
**A failure to *process*** — the handler failed. The message was fine; the work failed, and it
...
And with all of that going on, `go run ./cmd/sender` must still place a good order
