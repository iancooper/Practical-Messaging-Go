...
if command -v go >/dev/null 2>&1; then
  ...
  gmaj=${gv%%.*}; gmin=$(printf '%s' "$gv" | cut -d. -f2)
  if [ "${gmaj:-0}" -gt 1 ] 2>/dev/null || { [ "${gmaj:-0}" -eq 1 ] && [ "${gmin:-0}" -ge 23 ]; } 2>/dev/null
  then ok "go $gv"
  else bad "go $gv -- the exercises target Go 1.23, install a newer Go"; fi

  # Exercise 3's Kafka client is a cgo binding to librdkafka, so it needs cgo switched on and
  # a C compiler. It ships librdkafka for macOS and for glibc Linux, so a Mac or an Ubuntu is
  # fine; Alpine and cross-compiles are not, and neither is a shell with CGO_ENABLED=0 set.
  if [ "$(go env CGO_ENABLED)" = "1" ]
  then ok "cgo enabled -- exercise 3's Kafka client is a cgo binding and needs it"
  else bad "CGO_ENABLED=$(go env CGO_ENABLED) -- exercise 3 needs cgo; set CGO_ENABLED=1"; fi
else
  bad "go not found -- install Go 1.23 or later"
fi

if command -v cc >/dev/null 2>&1
then ok "a C compiler on PATH -- exercise 3 links librdkafka"
else bad "no C compiler -- install the Xcode command line tools, or build-essential"; fi
...
if [ -n "$busy" ]; then
  ...
  printf '  (simplemessaging/channel.go and simpleeventing/stream.go). See README.md.\n'
fi
...
# This is the other slow part to do at home: the first build downloads every module the three
# exercises use and links librdkafka, and a venue network is a poor place to discover that.
for d in ../01-message-pump ../02-failing-well ../03-streams; do
  if (cd "$d" && go build ./... >/dev/null 2>&1)
  then ok "$(basename "$d") builds"
  else bad "$(basename "$d") does not build -- run 'go build ./...' in it to see why"; fi
done

# The take-home, reported but not counted -- see 'note' at the top of this file.
if (cd ../04-lookup && go build ./... >/dev/null 2>&1)
then ok "04-lookup builds (optional, take home)"; ex4="builds"
     ...
     ex4="DOES NOT BUILD -- run 'go build ./...' in 04-lookup to see why"; fi
