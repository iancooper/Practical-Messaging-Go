...
```
cd 03-streams
go run ./cmd/receiver        # RabbitMQ consumer AND Kafka producer
go run ./cmd/streamconsumer  # Kafka consumer, counts what it sees
go run ./cmd/sender          # one order
```
...
| file | |
|---|---|
| `model/placeorderhandler.go` | the handler. **Nothing in it is wrong** |
| `simpleeventing/consumer.go` | the stream's version of the pump. Read the comment at the top |
| `simpleeventing/stream.go` | the topology — and compare its length with `simplemessaging/channel.go` |

1. **`channel.go` declares four queues. `stream.go` declares one topic.** Exercise 2 used the
...
**Now do it the other way round.** Move the acknowledgement in `simplemessaging/messagepump.go`
so it happens *before* `p.handler.Handle(...)` instead of after. Reset, and run the probe again — but
...
```
go run ./cmd/sender slow  # then kill the receiver during the lookup, before the event
```
...
```
DUAL_WRITE_WINDOW=15 go run ./cmd/receiver  # terminal 1
go run ./cmd/sender                         # terminal 2, then look at ../00-setup/queues.sh
```
...
**Put the acknowledgement back after `p.handler.Handle(...)` before you go on.** Probes B and C do
...
```
../00-setup/reset.sh
go run ./cmd/streamconsumer    # terminal 1, leave it running
go run ./cmd/sender bad-event  # appends an unreadable record to the topic
```
...
```
go run ./cmd/receiver        # terminal 2
go run ./cmd/sender burst 6  # terminal 3
../00-setup/lag.sh
```
...
> probe's answer.** The poison record is keyed with a fixed string by `cmd/sender/`, so it
> lands on the same partition every time you run this. Each good order's event is keyed by the
> order id, and the publisher is wired up in `cmd/receiver/` — so those scatter across all
...
```
../00-setup/reset.sh
go run ./cmd/receiver        # terminal 1, then send a few good orders and stop it
go run ./cmd/sender burst 4  # terminal 2
```

Now change `simpleeventing.ConsumerGroup` in `simpleeventing/stream.go` to any new name and run the stream
consumer again.
