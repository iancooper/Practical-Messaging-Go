...
go run ./cmd/streamconsumer    # Kafka consumer, counts duplicates
go run ./cmd/sender            # one order
go run ./cmd/sender bad-event  # an unreadable record, straight onto the stream

DUAL_WRITE_WINDOW=15 go run ./cmd/receiver  # widens the gap so you can aim at it
```
...
| `model/placeorderhandler.go` | places the order, then publishes the event. Also correct |
