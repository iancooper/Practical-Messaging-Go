...
# Practical Messaging — Go exercises #
...
```
git clone <this repo>
cd Practical-Messaging-Go/00-setup
./prereqs.sh
```

You need **Docker** and **Go 1.23 or later**. That is all. No cloud account, no API key, and
...
> `localhost:` constants in `simplemessaging/channel.go` and `simpleeventing/stream.go`.
...
| `cmd/sender/`, `cmd/receiver/` | two console apps, plus `cmd/streamconsumer` in exercise 3 |
